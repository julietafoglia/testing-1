...
const setupFixture002 =
    require(rootPath + '/fixtures/common/rbac/save/save001');
const testFixture =
...
describe('{{BOOTSTRAP}} <SETUP> [[SS-AGENCY]] 011 >>> ' +
    'agency002 - agencyUser001 >>> ', function(done) {
    ...
    before('create user - minimum required', function(done) {
        ...
        sendBody001.email = 'agcSelfService001-'
    ...
    before('save rbac user - self-service agency', function(done) {
        ...
        request(targetServer)
            ...
            .then( function(res) {
                ...
                // spot check response
                expect(resOutput002.Agency).to.be.instanceof(Array);
                expect(/^[a-f0-9]{32}$/
                    .test(resOutput002.Agency[0].id)).to.be.true;
                expect(resOutput002.Agency[0].id)
                    .to.equal(sendBody002.Agency[0].id);
                expect(resOutput002.Agency[0].roles)
    ...
    after('write entity to json file', function(done) {
        ...
        const createdEntity = {
            type: 'user',
            permission: 'self-service-agency',
            apiPermissions: 'agencyDemand, agencyAudience',
