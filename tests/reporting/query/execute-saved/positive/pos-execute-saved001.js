...
describe('{{REPORTING}} <SMOKE> /query {execute-saved} @ADMIN >>> ' +
    '(+) execute saved query with valid id >>>', function() {
    ...
    before('Execute saved query', function(done) {
        request(targetServer)
            .post(util.format(targetEndpoint.queryExecute, queryOutput1.id))
            .set(authHeaders)
            .then( function(res) {
    ...
    it('execute saved query should have a response of 200', function(done) {
