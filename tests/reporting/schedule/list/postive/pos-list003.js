...
describe('{{REPORTING}} <SMOKE> /schedule report @ADMIN >>> ' +
    '(+) get one scheduled report for a given id >>>', function() {
    ...
    before('Get existing user scheduled reports', function(done) {
    ...
    before('get one scheduled report for a given id', function(done) {

        request(targetServer)
            .get(util.format(targetEndpoint.scheduleRead, queryOutput1.data.id))
            .set(authHeaders)
            .then( function(response) {
                // basic response verification
                expect(response.body).to.exist;
