...
describe('{{MERLIN}} /agency/{id delete} @ADMIN >>> ' +
    '(-) request - POST - valid body >>>', function() {
    ...
    before('delete agency - POST - valid body', function(done) {
        sendBody002 = {};
        sendBody002.version = resOutput001.version;

        request(targetServer)
            .post(util.format(targetEndpoint.agencyDelete, resOutput001.id))
            .set(authHeaders)
            .send(sendBody002)
            .then( function(res) {
                // basic response verification
    ...
    it('agency status should be deleted', function() {
        expect(resOutput003.status).to.not.equal('deleted');
    });
