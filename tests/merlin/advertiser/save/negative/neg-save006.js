...
describe('{{MERLIN}} /advertiser {id save} @ADMIN >>> ' +
    ...
    before('save advertiser - multiple invalid fields', function(done) {

        sendBody003 = {};

        // assign version to advertiser
        sendBody003.version = resOutput002.version;

        // assign invalid fields to advertiser
        sendBody003.category =
            chance.string({length: 5, pool: characterPool});
        sendBody003.categories =
            [chance.string({length: 5, pool: characterPool})];
        sendBody003.admin =
            'ghost' + chance.hash({length: 27});
        sendBody003.executive =
            'ghost' + chance.hash({length: 27});
        sendBody003.name =
            chance.string({length: 256, pool: characterPool});
        sendBody003.externalId =
            chance.string({length: 129, pool: characterPool});
        sendBody003.liveramp =
            chance.string({length: 129, pool: characterPool});
        sendBody003.domain =
            chance.string({length: 129, pool: characterPool});
        sendBody003.targetingType =
            chance.string({length: 5, pool: characterPool});
        sendBody003.targetedPublishers =
            ['ghost' + chance.hash({length: 27})];
        sendBody003.targetedDomains =
            [chance.string({length: 129, pool: characterPool})];
        sendBody003.suppressCompetitive =
            chance.string({length: 5, pool: characterPool});
        sendBody003.contract = {};
        sendBody003.contract.dspFee =
            chance.string({length: 5, pool: characterPool});
