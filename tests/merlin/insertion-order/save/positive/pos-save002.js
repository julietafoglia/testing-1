...
const testFixture =
    require(rootPath + '/fixtures/common/insertion-order/save003');
...
describe('{{MERLIN}} <SMOKE> /insertion-order {id save} @ADMIN >>> ' +
    ...
    before('save insertion order - all valid fields', function(done) {

        sendBody004 = Object.assign({}, testFixture);

        // assign name, version and new flight dates
        sendBody004.name = resOutput003.name + '@v2';
        sendBody004.version = resOutput003.version;
        sendBody004.startDate = moment().add(10, 'days')
            .format('YYYY-MM-DD HH:mm:ss');
        sendBody004.endDate = moment().add(25, 'days')
            .format('YYYY-MM-DD HH:mm:ss');

        // assign random values to empty fields
        sendBody004.referenceNumber =
            chance.string({length: 10, characterPool});
        sendBody004.agencyCampaign =
            chance.string({length: 10, characterPool});
    ...
    it('response object key values should match test object', function() {
        ...
        expect(resOutput004.isUnlimitedBudget)
            .to.equal(sendBody004.isUnlimitedBudget); // check updated
        ...
        expect(resOutput004.startDate)
            .to.equal(sendBody004.startDate); // check updated
        expect(resOutput004.endDate)
            .to.equal(sendBody004.endDate); // check updated
