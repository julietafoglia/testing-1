...
describe('{{MERLIN}} /newsletter {id delete} >>> ' +
    '(+) url - ends in a trailing forward slash >>>', function() {
    ...
    before('delete newsletter - url end trailing slash', function(done) {
        request(targetServer)
            .del(util.format(
                targetEndpoint.newsletterDelete, resOutput003.id + '/'
            )
            )
            .set(authHeaders)
            .then( function(res) {
    ...
    before('get newsletter - basic fields', function(done) {

        request(targetServer)
            .get(util.format(targetEndpoint.newsletterDetails, resOutput003.id))
            .set(authHeaders)
            .then( function(res) {
                // basic response verification
                expect(res.body).to.exist;
    ...
    it("response object's status fields should be deleted", function() {
