...
const testFixture =
    require(rootPath + '/fixtures/common/newsletter/create003');
const verifyFixture =
    require(rootPath + '/fixtures/common/newsletter/create003-verify');
...
describe('{{MERLIN}} /newsletter {create} >>> ' +
    ...
    before('create newsletter - null non-required fields', function(done) {
    ...
    it('response object key values should match test object', function() {
        // should match test object or have the default value
        expect(resOutput003.publisher)
            .to.equal(sendBody003.publisher);
        ...
        expect(resOutput003.estimatedInventory)
            .to.equal(verifyFixture.estimatedInventory);
        expect(resOutput003.isTakeover)
            .to.equal(verifyFixture.isTakeover);
        expect(resOutput003.isRoadblock)
            .to.equal(verifyFixture.isTakeover);
        // ssp-control object
        expect(resOutput003.sspControl.exchangeAllow)
        ...
        expect(resOutput003.sspControl.uniqueAds)
            .to.equal(verifyFixture.sspControl.uniqueAds);
