...
describe('{{MERLIN}} /newsletter {create} >>> ' +
    ...
    before('create newsletter - string fields max length', function(done) {

        sendBody003 = Object.assign({}, testFixture);

        // assign publisher to newsletter
        sendBody003.publisher = resOutput002.id;

        // assign max length to string fields
        sendBody003.name += timeStamp + chance.string({length: 11});
        sendBody003.externalId = chance.string({length: 128});
        sendBody003.description = sendBody003.name;
