...
describe('{{MERLIN}} /newsletter {create} >>> ' +
    '(-) request - PUT with a valid body >>>', function() {
    ...
    before('create newsletter - PUT - valid body', function(done) {
        ...
        request(targetServer)
            .put(util.format(targetEndpoint.newsletterCreate))
            .set(authHeaders)
            .send(sendBody003)
            .then( function(res) {
                // basic response verification
    ...
    it('response should have status of 404', function() {
        expect(res003.status).to.equal(404);
    });


    it('response should contain a not found error', function() {
        expect(resText003.statusCode).to.equal(404);
        expect(resText003.error).to.equal('Not Found');
    });
