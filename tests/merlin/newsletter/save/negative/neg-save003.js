...
describe('{{MERLIN}} /newsletter {id save} >>> ' +
    ...
    before('save newsletter - invalid basic fields', function(done) {

        sendBody004 = {};

        // assign version to newsletter
        sendBody004.version = resOutput003.version;

        // assign invalid basic fields to newsletter
        sendBody004.publisher =
            'ghost' + chance.hash({length: 27});
        sendBody004.category =
            chance.string({length: 30, pool: characterPool});
        sendBody004.secondaryCategories =
            [chance.string({length: 2, pool: characterPool})];
