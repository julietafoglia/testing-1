...
const testFixture =
    require(rootPath + '/fixtures/common/newsletter/save002');
...
describe('{{MERLIN}} <SMOKE> /newsletter {id save} >>> ' +
    '(+) body - all valid fields verification >>>', function() {
    ...
    before('save newsletter - all valid fields', function(done) {

        sendBody004 = Object.assign({}, testFixture);

        // assign name and version to newsletter
        sendBody004.name = resOutput001.name + '@v2';
    ...
    it('response object key values should match test object', function() {
        ...
        expect(resOutput004.category)
            .to.equal(sendBody004.category);
        expect(resOutput004.secondaryCategories)
            .to.eql(sendBody004.secondaryCategories);
        expect(resOutput004.name)
        ...
        expect(resOutput004.estimatedInventory)
            .to.equal(sendBody004.estimatedInventory);
        // ssp-control object
        expect(resOutput004.sspControl.exchangeAllow)
        ...
        expect(resOutput004.sspControl.rtbAllow)
            .to.equal(sendBody004.sspControl.rtbAllow);
        expect(resOutput004.sspControl.rtbTransparency)
            .to.equal(sendBody004.sspControl.rtbTransparency);
        expect(resOutput004.sspControl.rtbFloor)
            .to.equal(sendBody004.sspControl.rtbFloor);
        expect(resOutput004.sspControl.uniqueAds)
            .to.equal(sendBody004.sspControl.uniqueAds);
        expect(resOutput004.sspControl.userMatchAllow)
            .to.equal(sendBody004.sspControl.userMatchAllow);
        expect(resOutput004.sspControl.demandAllocationHouse)
            .to.equal(sendBody004.sspControl.demandAllocationHouse);
        expect(resOutput004.sspControl.demandAllocationDirect)
            .to.equal(sendBody004.sspControl.demandAllocationDirect);
        expect(resOutput004.sspControl.directoryExpose)
            .to.equal(sendBody004.sspControl.directoryExpose);
        expect(resOutput004.sspControl.directoryExposePublic)
            .to.equal(sendBody004.sspControl.directoryExposePublic);
    });
