...
describe('{{MERLIN}} /strategy/{id delete} @ADMIN >>> ' +
    '(+) url - media-group - id - random >>>', function() {
    ...
    before('delete strategy - id - random', function(done) {
        request(targetServer)
            .del(util.format(
                targetEndpoint.strategyDelete,
    ...
    it('response have status of 400', function() {
        expect(res007.status).to.equal(400);
    });

    it('errors should include invalid id', function() {
        expect(resText007.errors).to.deep.include.members([
