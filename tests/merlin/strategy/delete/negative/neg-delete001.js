...
describe('{{MERLIN}} /strategy/{id delete} @ADMIN >>> ' +
    '(+) url - media-group - should not be writable >>>', function() {
    ...
    before('delete strategy', function(done) {
        request(targetServer)
            .del(util.format(
    ...
    before('save strategy', function(done) {
        sendBody007 = {};
        sendBody007.version = resOutput005.version;
        sendBody007.name = timeStamp + '@v2';

        // assign campaign
        sendBody007.campaign = resOutput004.id;

        request(targetServer)
            .post(util.format(targetEndpoint.strategySave, resOutput005.id))
            .set(authHeaders)
            .send(sendBody007)
            .then( function(res) {
                // basic response verification
    ...
    it('response should have status of 410', function() {
        expect(res007.status).to.equal(410);
    });
