...
describe('{{MERLIN}} /strategy/{id delete} @ADMIN >>> ' +
    '(+) url - media-group - status - should be deleted >>>', function() {
    ...
    it('response should have status of 200', function() {
        expect(res007.status).to.equal(200);
    });
    ...
    it('agency status should be deleted', function() {
        expect(resOutput007.status).to.equal('deleted');
    });
