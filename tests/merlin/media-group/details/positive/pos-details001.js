...
describe('{{MERLIN}} <SMOKE> /media-group {id details} @ADMIN >>> ' +
    '(+) url - basic verification - hash id >>>', function() {
    ...
    before('get media-group details - hash id', function(done) {
        request(targetServer)
            .get(util.format(targetEndpoint.mediaGroupDetails, resOutput001.id))
            .set(authHeaders)
            .then( function(res) {
                // basic response verification
                expect(res.body).to.exist;
    ...
    it('response object key values should match test object', function() {
        ...
        // created and modified
        expect(resOutput002.created)
            .to.equal(resOutput001.created);
        expect(resOutput002.createdBy)
            .to.equal(resOutput001.createdBy);
        expect(resOutput002.modified)
            .to.equal(resOutput001.modified);
        expect(resOutput002.modifiedBy)
            .to.equal(resOutput001.modifiedBy);
    });
