...
const verifyFixture =
    require(rootPath + '/fixtures/common/media-group/create004-verify');
...
describe('{{MERLIN}} <SMOKE> /media-group {create} @ADMIN >>> ' +
    '(+) body - all valid fields - monetization >>>', function() {
    ...
    before('create media-group - all valid fields', function(done) {

        sendBody001 = Object.assign({}, testFixture);

        // assign name and contract type
        sendBody001.name += timeStamp;
        // sendBody001.contract.type = 'Monetization';

        // assign random lotame and salesforce-id
        sendBody001.lotame =
            chance.string({length: 40, pool: characterPool});
        sendBody001.salesforceId =
            chance.string({length: 30, pool: characterPool});
