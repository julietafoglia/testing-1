...
describe('{{MERLIN}} /ad-slot {id save} >>> ' +
    ...
    before('save ad-slot - PUT valid body', function(done) {
        ...
        request(targetServer)
            .put(util.format(targetEndpoint.adSlotSave, resOutput004.id))
            .set(authHeaders)
            .send(sendBody005)
            .then( function(res) {
                // basic response verification
    ...
    it('response have status of 404', function() {
        expect(res005.status).to.equal(404);
    });

    it('response should contain a not found error', function() {
        expect(resText005.statusCode).to.equal(404);
        expect(resText005.error).to.equal('Not Found');
    });
