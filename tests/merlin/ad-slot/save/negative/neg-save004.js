...
describe('{{MERLIN}} /ad-slot {id save} >>> ' +
    '(-) body - save a to previously saved version >>>', function() {
    ...
    before('save ad-slot - update name', function(done) {

        sendBody005 = {};

        // assign version and name to ad-slot
    ...
    before('save ad-slot - previous version', function(done) {

        sendBody006 = {};

        // assign name and a previous version to ad-slot
        sendBody006.name = 'ads@' + moment().format('YYYY-MM-DDTHH:mm:ss.SS');
        sendBody006.version = resOutput004.version;
    ...
    it('response have status of 409', function() {
        expect(res006.status).to.equal(409);
    });
