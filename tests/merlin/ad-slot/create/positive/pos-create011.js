...
const testFixture =
    require(rootPath + '/fixtures/common/ad-slot/create007');
...
describe('{{MERLIN}} /ad-slot {create} >>> ' +
    '(+) native ad-slots - prod listing - outside content >>>', function() {
    ...
    it('response object property types should match spec', () => {
        ...
        expect(resOutput004.urlImage).to.be.an('array');
        expect(resOutput004.urlClick).to.be.an('array');
        expect(resOutput004.urlUnsubscribe).to.be.a('string');
        expect(resOutput004.tag).to.be.an('array');
