...
describe('{{MERLIN}} /ad-slot {create} >>> ' +
    ...
    before('create ad-slot - multiple invalid fields', function(done) {

        sendBody004 = {};

        // assign newsletter to ad-slot
        sendBody004.newsletter = resOutput003.id;

        // assign invalid fields to ad-slot
        ...
        sendBody004.mediaType =
            chance.string({length: 5, pool: characterPool});
        sendBody004.position =
            chance.string({length: 5, pool: characterPool});
        sendBody004.width =
            chance.string({length: 2, alpha: true});
        sendBody004.height =
            chance.string({length: 2, alpha: true});
        sendBody004.sspFee =
            chance.string({length: 5, pool: characterPool});
        ...
        sendBody004.sspControl.exchangeFloor =
            chance.string({length: 10, pool: characterPool});
        sendBody004.sspControl.targetingType =
            chance.string({length: 10, pool: characterPool});
    ...
    it('response should contain errors for each invalid field', function() {
        expect(resText004.errors).to.deep.include.members([
            ...
            {'id':'E1000', 'code':'INV', 'details':'mediaType'},
            {'id':'E1000', 'code':'INV', 'details':'position'},
            {'id':'E1000', 'code':'INV', 'details':'sspFee'},
            {'id':'E1000', 'code':'INV', 'details':'sspControl:exchangeAllow'},
            {'id':'E1000', 'code':'INV', 'details':'sspControl:exchangeFloor'},
            {'id':'E1000', 'code':'INV', 'details':'sspControl:targetingType'},
            {'id':'E1001', 'code':'REQ', 'details':'sizes'}
        ]);
    });
