...
const testFixture =
    require(rootPath + '/fixtures/common/bundle/save002');
...
describe('{{MERLIN}} <SMOKE> /bundle {id save} >>> ' +
    '(+) body - update all valid fields >>>', function() {
    ...
    before('update bundle - all valid fields', function(done) {
        ...
        request(targetServer)
            .post(util.format(targetEndpoint.bundleSave, resOutput003.id))
            .set(authHeaders)
            .send(sendBody004)
            .then( function(res) {
                // basic response verification
    ...
    it('response object property types should match spec', function() {
        ...
        expect(resOutput004.name).to.be.a('string');
        expect(resOutput004.name).to.have.length.of.at.most(32);
        ...
        if (resOutput004.publisher !== null) {
            expect(/^[a-f0-9]{32}$/.test(resOutput004.publisher)).to.be.true;
        }
        if (resOutput004.adSlots !== null) {
            expect(resOutput004.adSlots).to.be.an('array');
            resOutput004.adSlots.forEach(function(val) {
                expect(/^[a-f0-9]{32}$/.test(val)).to.be.true;
            });
        }
        if (resOutput004.strategies !== null) {
            expect(resOutput004.strategies).to.be.an('array');
            resOutput004.strategies.forEach(function(val) {
                expect(/^[a-f0-9]{32}$/.test(val)).to.be.true;
            });
        }
        if (resOutput004.isRoadblock !== null) {
            expect(resOutput004.isRoadblock).to.be.a('boolean');
        }
    ...
    it('response object should match test object', function() {
        // updated values compared to sendBody004
        ...
        expect(resOutput004.description).to.equal(sendBody004.description);
