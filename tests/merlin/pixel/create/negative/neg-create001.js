...
describe('{{MERLIN}} /pixel {create} @ADMIN >>> ' +
    '(-) body - invalid fields >>>', function() {
    ...
    before('create pixel - invalid fields', function(done) {
        // create request with invalid fields
        sendBody001 = {};
        sendBody001.advertiser = 'ghost' + chance.hash({length: 27});
        sendBody001.type = chance.word({length: 7});
        sendBody001.dspConversionType = chance.word({length: 10});
    ...
    it('notices should not exist', function() {
    ...
    it('response should contain errors for each invalid field', function() {
        expect(resText001.errors).to.deep.include.members([
            {'id': 'E1000', 'code': 'INV', 'details': 'dspConversionType'},
