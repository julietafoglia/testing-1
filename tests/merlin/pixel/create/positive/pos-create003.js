...
describe('{{MERLIN}} /pixel {create} @ADMIN >>> ' +
    '(+) body - string fields of maximum length >>>', function() {
    ...
    before('create pixel - string fields of maximum length', function(done) {
        sendBody003 = Object.assign({}, testFixture);

        // assign max length name to pixel
        sendBody003.name = 'pixel@' + moment().format('YYYY-MM-DDTHH:mm:ss.SS')
             + chance.word({length: 20}); // change to 227 lenght after SVC-4384
