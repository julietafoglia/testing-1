...
const testFixture =
    require(rootPath + '/fixtures/common/publisher/save001');
...
describe('{{MERLIN}} /publisher {id save} >>> ' +
    ...
    before('create publisher - all valid fields', function(done) {
        ...
        // add random values to empty fixture fields
    ...
    before('save publisher - strings maximum length', function(done) {

        sendBody003 = Object.assign({}, testFixture);

        // assign media-group and version to publisher
        sendBody003.mediaGroup = resOutput001.id;
        sendBody003.version = resOutput002.version;

        // create max length string fields
        sendBody003.name = chance.word({length: 255});
        sendBody003.externalId = chance.word({length: 32});
        sendBody003.tagsUrlPrefix = chance.word({length: 128});
        sendBody003.emailTagReplacement = chance.word({length: 128});
        sendBody003.placementTagReplacement = chance.word({length: 128});
        sendBody003.targetedDomains = sendBody002.targetedDomains;

        sendBody003.contact = {};
        sendBody003.contact.company = chance.word({length: 128});
        sendBody003.contact.firstName = chance.word({length: 32});
        sendBody003.contact.lastName = chance.word({length: 32});
        sendBody003.contact.phone =
            chance.string({length: 32, pool: '0123456789'});
        sendBody003.contact.email = testFixture.contact.email;
        sendBody003.contact.address1 = chance.word({length: 128});
        sendBody003.contact.address2 = chance.word({length: 64});
        sendBody003.contact.city = chance.word({length: 64});
        sendBody003.contact.state = chance.word({length: 32});
        sendBody003.contact.postalCode = chance.word({length: 32});
        sendBody003.contact.country = chance.word({length: 32});
    ...
    it('response object key values should match test object', function() {
        ...
        expect(resOutput003.emailTagReplacement)
            .to.equal(sendBody003.emailTagReplacement);
        expect(resOutput003.placementTagReplacement)
            .to.equal(sendBody003.placementTagReplacement);
        expect(resOutput003.contactEmails)
            .to.eql(sendBody003.contactEmails);
        expect(resOutput003.categories)
            .to.eql(sendBody003.categories);
        expect(resOutput003.blocklistCategories)
            .to.eql(sendBody003.blocklistCategories);
        expect(resOutput003.iabCreativeAttribute)
            .to.eql(sendBody003.iabCreativeAttribute);
        expect(resOutput003.targetedAdvertisers)
            .to.eql(sendBody003.targetedAdvertisers);
        expect(resOutput003.targetedDomains)
            .to.eql(sendBody003.targetedDomains);
        expect(resOutput003.keyValues)
            .to.eql(sendBody003.keyValues);
        expect(resOutput003.keyValuesCount)
            .to.equal(sendBody003.keyValuesCount);
        // ssp control object
        ...
        expect(resOutput003.sspControl.rtbTransparency)
            .to.equal(sendBody003.sspControl.rtbTransparency);
        ...
        expect(resOutput003.contact.company)
            .to.equal(sendBody003.contact.company);
        expect(resOutput003.contact.email)
            .to.equal(sendBody003.contact.email);
        expect(resOutput003.contact.firstName)
            .to.equal(sendBody003.contact.firstName);
        expect(resOutput003.contact.lastName)
            .to.equal(sendBody003.contact.lastName);
        expect(resOutput003.contact.lastName)
            .to.equal(sendBody003.contact.lastName);
        expect(resOutput003.contact.phone)
            .to.equal(sendBody003.contact.phone);
        expect(resOutput003.contact.address1)
            .to.equal(sendBody003.contact.address1);
        expect(resOutput003.contact.address2)
            .to.equal(sendBody003.contact.address2);
        expect(resOutput003.contact.city)
            .to.equal(sendBody003.contact.city);
        expect(resOutput003.contact.state)
            .to.equal(sendBody003.contact.state);
        expect(resOutput003.contact.postalCode)
            .to.equal(sendBody003.contact.postalCode);
        expect(resOutput003.contact.country)
            .to.equal(sendBody003.contact.country);
    });
