...
describe('{{MERLIN}} /publisher {id save} >>> ' +
    ...
    before('save publisher - url end blank', function(done) {

        sendBody003 = {};

        // assign version to publisher
        sendBody003.version = resOutput002.version;

        request(targetServer)
            .post(util.format(
                targetEndpoint.publisherSave, resOutput002.id + '  '
            )
            )
            .set(authHeaders)
            .send(sendBody003)
    ...
    it('response should not contain any errors or notices', function() {
