...
describe('{{MERLIN}} /publisher {id save} >>> ' +
    ...
    before('save publisher - invalid version', function(done) {

        sendBody003 = Object.assign({}, testFixture);

        // assign name and an invalid version to publisher
        sendBody003.name = resOutput002.name + '@v2';
        sendBody003.version = chance.string({length: 5});
    ...
    it('response should contain a invalid version error', function() {
        expect(resText003.errors).to.deep.include.members([
            {'id': 'E1000', 'code': 'INV', 'details': 'version'}
        ]);
    });
