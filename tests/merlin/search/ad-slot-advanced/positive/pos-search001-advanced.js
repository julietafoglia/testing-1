...
describe('{{MERLIN}} /search/ad-slot {advanced} @ADMIN >>> ' +
    ...
    it('response object property types should match spec', function() {
        ...
        expect(resOutput001[0].publisherName).to.have.length.of.at.most(255);
        expect(/^[a-f0-9]{32}$/.test(resOutput001[0].newsletter)).to.be.true;
        if (resOutput001[0].adSlot !== null) {
            expect(/^[a-f0-9]{32}$/.test(resOutput001[0].adSlot)).to.be.true;
        }
        if (resOutput001[0].externalId !== null) {
            expect(resOutput001[0].externalId).to.have.length.of.at.most(128);
        }
        if (resOutput001[0].type !== null) {
        ...
        expect(/^[0-9]{1,32}$/.test(resOutput001[0].modifiedBy)).to.be.true;
    });
});
