...
let testFixture = require(rootPath +
    '/fixtures/common/search/ssp-control-publisher/parameters003');
...
describe('{{MERLIN}} /search/ssp-control/publisher {advanced} @ADMIN >>> ' +
    '(+) body - mode verification - or, and >>>', function() {
    ...
    it('response should match search criteria', function() {
        expect(resOutput001.length).to.be.at.most(sendBody001.number);
        resOutput001.forEach(function(value) {
            expect(value.uniqueAds).to.be.oneOf(['advertiser', 'inherit']);
            if (value.uniqueAds === 'advertiser') {
                expect(value.userMatchAllow).to.equal(false);
            }
            if (value.uniqueAds === 'inherit') {
