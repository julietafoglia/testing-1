...
let testFixture = require(rootPath +
    '/fixtures/common/search/ssp-control-publisher/parameters001');
...
describe('{{MERLIN}} <SMOKE> /search/ssp-control/publisher' +
    ...
    it('response object should match search parameters', function() {
        expect(resOutput001.length).to.be.at.most(sendBody001.number);
        resOutput001.forEach(function(value) {
            expect(value.targetingType).to.be.oneOf(
                ['include', 'exclude']
            );
            if (value.targetingType === 'exclude') {
                expect(value.uniqueAds).to.equal('off');
            }
        });
    });
