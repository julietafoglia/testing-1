...
describe('{{MERLIN}} /search/ssp-control/publisher {advanced} @ADMIN >>> ' +
    '(+) body - order-by verification - rtb-floor >>>', function() {
    ...
    it('response should be ordered by rtbFloor in ascending order', function() {
        expect(resOutput001.length).to.be.at.most(sendBody001.number);
        let previous = 0;
        resOutput001.forEach(function(value) {
            expect(value.rtbFloor).to.be.at.least(previous);
