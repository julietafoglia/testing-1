...
describe('{{MERLIN}} /search/strategy/newsletter {advanced} >>> ' +
    '(+) newsletter - no linked strategies >>>', function() {
    ...
    before('search linked strategies', (done) => {
        ...
        let payload = {
            'conditions': [
                {'field': 'newsletter', 'value': resOutput003.id}
            ]
        };

        request(targetServer)
            .post(util.format(targetEndpoint.searchStrategyNewsletterAdvanced))
            .send(payload)
            .set(authHeaders)
            .then((res) => {
                // basic response verification
    ...
    it('should have no linked strategies', () => {
        expect(resOutput004).to.have.lengthOf(0);
    });
