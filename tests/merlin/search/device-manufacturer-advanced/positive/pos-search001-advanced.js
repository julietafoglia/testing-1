...
describe('{{MERLIN}} /search/device-manufacturer {advanced} @ADMIN >>> ' +
    ...
    it('search device-manufacturer - ' +
        'order by id - asc - number 5 - page 1', function(done) {

        sendBody001 = Object.assign(searchFixture);
        sendBody001.sort = 'asc';

        request(targetServer)
            .post(util.format(
                targetEndpoint.searchDeviceManufacturerAdvanced
            )
            )
            .set(authHeaders)
            .send(sendBody001)
