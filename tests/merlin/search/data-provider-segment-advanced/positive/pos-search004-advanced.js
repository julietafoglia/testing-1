...
describe('{{MERLIN}} /search/data-provider/segment {advanced} @ADMIN >>> ' +
    ...
    'return id name source - condition and eq rapleaf >>>', function() {
    ...
    before('search data-provider segment - ' +
        'order by name - desc - number 50 - page 1 - ' +
        'return id name source - condition eq rapleaf', function(done) {
        ...
        sendBody001.conditions = [
            {'field': 'source', 'operator': 'eq', 'value': 'rapleaf'}
        ];
    ...
    it('array first object should only contain id ' +
        'name and status key values', function() {
        expect(resOutput001[0])
            .to.contain.all.keys('id', 'name', 'source');
