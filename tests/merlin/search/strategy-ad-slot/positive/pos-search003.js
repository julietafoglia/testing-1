...
describe('{{MERLIN}} /search/strategy/ad-slot >>> ' +
    ...
    before('create ad-slot - all valid fields', (done) => {
        ...
        // bug reported SUP-679 strategy shouldn't be linked
    ...
    it('link strategy to ad-slot', (done) => {
        let payload = {
            'version': resOutput008.version + 1,
            'adSlots': [resOutput004.id]
        };
        request(targetServer)
            .post(
                util.format(
                    targetEndpoint.strategyLinkAdSlots, resOutput008.id
                )
            )
            .send(payload)
            .set(authHeaders)
