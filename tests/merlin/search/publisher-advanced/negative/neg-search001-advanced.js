...
describe('{{MERLIN}} /search/publisher {advanced} @ADMIN >>> ' +
    ...
    before('search publisher - body - missing', function(done) {

        // missing request send body

        request(targetServer)
            .post(util.format(targetEndpoint.searchPublisherAdvanced))
            .set(authHeaders)
            .then( function(res) {
    ...
    it('response object property types should match spec', function() {
        ...
        expect(/^[a-f0-9]{32}$/.test(resOutput001[0].mediaGroup)).to.be.true;
        expect(resOutput001[0].mediaGroupName).to.have.length.of.at.most(255);
        if (resOutput001[0].esp !== null) {
            expect(validator.isInt(resOutput001[0].esp + '')).to.be.true;
        }
        ...
        if (!/[0-9]+/.test(resOutput001[0].modifiedBy)) {
            expect(/^[a-f0-9]{32}$/.test(resOutput001[0].modifiedBy))
                .to.be.true;
        }
    });
});
