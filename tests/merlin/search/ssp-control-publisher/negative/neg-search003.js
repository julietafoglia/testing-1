...
describe('{{MERLIN}} /search/ssp-control/publisher @ADMIN >>> ' +
    '(-) url - unsupported query parameters >>>', function() {
    ...
    before('search ssp-control-publisher - unsupported param', function(done) {

        const urlParams = [
            'rtbFloor', 0,
            'demandAllocationHouse', 2,
            'demandAllocationDirect', 5
        ].join('/');
    ...
    it('response should contain unsupported field errors', function() {
        expect(resText001.errors).to.deep.include.members([
            {'id': 'E1002', 'code': 'UNS',
                'details': 'condition:rtbFloor'},
            {'id': 'E1002', 'code': 'UNS',
                'details': 'condition:demandAllocationHouse'},
            {'id': 'E1002', 'code': 'UNS', 'details':
                'condition:demandAllocationDirect'}
        ]);
    });
