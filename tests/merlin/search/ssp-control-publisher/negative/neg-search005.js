...
describe('{{MERLIN}} /search/ssp-control/publisher @ADMIN >>> ' +
    '(-) url - invalid order-by parameter - publisher-id >>>', function() {
    ...
    before('search ssp-control-publisher - invalid parameter', function(done) {
    ...
    it('response should contain an error for each invalid param', function() {
        expect(resText001.errors).to.deep.include.members([
            {'id': 'E1000', 'code': 'INV', 'details': 'o:publisherId'}
        ]);
    });
