...
describe('{{MERLIN}} <SMOKE> /search/agency @ADMIN >>> ' +
    ...
    before('search agency', function(done) {
        request(targetServer)
            .get(util.format(targetEndpoint.searchAgency))
            .set(authHeaders)
            .then( function(res) {
    ...
    it('response object property types should match spec', function() {
        ...
        expect(resOutput001[0].managerName).to.have.length.of.at.most(50);
        expect(validator.isEmail(resOutput001[0].managerEmail)).to.be.true;
