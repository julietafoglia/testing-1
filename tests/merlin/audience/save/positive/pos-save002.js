...
const setupFixture003 =
    require(rootPath + '/fixtures/common/audience/create002');
const testFixture =
    require(rootPath + '/fixtures/common/audience/save001');
...
describe('{{MERLIN}} <SMOKE> /audience {id save} >>> ' +
    ...
    before('save audience - all valid fields', function(done) {

        sendBody005 = Object.assign({}, testFixture);

        // assign version and name to audience
        sendBody005.version = resOutput004.version;
        sendBody005.name = resOutput004.name + '@v2';
        sendBody005.dataProviderId = resOutput003[0].id;
        sendBody005.isShared = true;
    ...
    it('response object key values should match test object(s)', function() {
        // compare updated fields to sendBody004
        expect(resOutput005.advertiser).to.equal(resOutput004.advertiser);
        expect(resOutput005.name).to.equal(sendBody005.name);
        expect(resOutput005.records).to.equal(resOutput004.records);
        expect(resOutput005.externalId).to.equal(sendBody005.externalId);
        expect(resOutput005.isShared).to.equal(sendBody005.isShared);
        expect(resOutput005.isDds).to.equal(sendBody005.isDds);
