...
describe('{{MERLIN}} <SMOKE> /audience {id details} >>> ' +
    '(+) url - all valid fields verification >>>', function() {
    ...
    before('create audience - all valid fields', function(done) {
    ...
    before('get audience - all valid fields verification', function(done) {
        request(targetServer)
            .get(util.format(targetEndpoint.audienceDetails, resOutput004.id))
            .set(authHeaders)
            .then( function(res) {
                // basic response verification
                expect(res.body).to.exist;
    ...
    it('response object key values should match created object', function() {
        ...
        expect(resOutput005.isDds).to.equal(resOutput004.isDds);
