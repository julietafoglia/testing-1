...
describe('{{MERLIN}} /audience {id details} >>> ' +
    ...
    before('create audience - minimum required fields', function(done) {
        ...
        request(targetServer)
            ...
            .then( function(res) {
                ...
                resOutput005 = resText004.output;

                // spot check response
                expect(/^[a-f0-9]{32}$/.test(resOutput005.id))
                    .to.be.true;
    ...
    before('get audience - basic fields verification', function(done) {
        request(targetServer)
            .get(util.format(targetEndpoint.audienceDetails, resOutput005.id))
            .set(authHeaders)
            .then( function(res) {
                // basic response verification
                expect(res.body).to.exist;
    ...
    it('response object property types should match spec', function() {
        ...
        expect(resOutput005.isDds).to.be.a('boolean');
    ...
    it('response object key values should match created object', function() {
        expect(resOutput005.id).to.equal(resOutput005.id);
        expect(resOutput005.refId).to.equal(resOutput005.refId);
        expect(resOutput005.version).to.equal(resOutput005.version);
        expect(resOutput005.advertiser).to.equal(resOutput005.advertiser);
        expect(resOutput005.name).to.equal(resOutput005.name);
        expect(resOutput005.records).to.equal(resOutput005.records);
        expect(resOutput005.externalId).to.equal(resOutput005.externalId);
        expect(resOutput005.isShared).to.equal(resOutput005.isShared);
        expect(resOutput005.isDds).to.equal(resOutput005.isDds);
        expect(resOutput005.uploaded).to.equal(resOutput005.uploaded);
        expect(resOutput005.created).to.equal(resOutput005.created);
        expect(resOutput005.createdBy).to.equal(resOutput005.createdBy);
        expect(resOutput005.modified).to.equal(resOutput005.modified);
        expect(resOutput005.modifiedBy).to.equal(resOutput005.modifiedBy);
    });

    after('delete audience', function(done) {
        request(targetServer)
            .del(util.format(targetEndpoint.audienceDelete, resOutput005.id))
            .set(authHeaders)
            .then( function(res) {
                // basic response verification
                expect(res.body).to.exist;
