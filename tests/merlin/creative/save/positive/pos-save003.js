...
const testFixture =
    require(rootPath + '/fixtures/common/creative/save002');
...
describe('{{MERLIN}} <SMOKE> /creative/{id save} @ADMIN >>> ' +
    ...
    before('save creative - all valid fields', function(done) {
        sendBody004 = {};
        Object.assign(
            sendBody004,
            testFixture
    ...
    it('response object key values should match test object', function() {
        ...
        expect(resOutput004.isServices)
            .to.equal(sendBody004.isServices); // check updated
        expect(resOutput004.frequencyCapPeriod)
            .to.equal(sendBody004.frequencyCapPeriod); // check updated
        expect(resOutput004.frequencyCapCount)
            .to.equal(sendBody004.frequencyCapCount); // check updated
        ...
        expect(resOutput004.bidderVisibility)
            .to.equal(sendBody004.bidderVisibility); // check updated
        expect(resOutput004.urlCachedCopy)
            .to.equal(resOutput003.urlCachedCopy);
        expect(resOutput004.clickUrl)
            .to.equal(sendBody004.clickUrl); // check updated
        expect(resOutput004.iabAttributes)
            .to.eql(sendBody004.iabAttributes); // check updated
        expect(resOutput004.urltracking1)
            .to.equal(sendBody004.urltracking1); // check updated
        expect(resOutput004.urltracking2)
            .to.equal(sendBody004.urltracking2); // check updated
