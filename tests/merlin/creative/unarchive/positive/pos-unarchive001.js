...
describe('{{MERLIN}} <SMOKE> /creative/unarchive/{id} @ADMIN >>> ' +
    '(+) body - basic verification - url file >>>', function() {
    ...
    before('unarchive creative', function(done) {
        sendBody005 = {};
        sendBody005.version = resOutput004.version;

        request(targetServer)
            .post(util.format(
                targetEndpoint.creativeUnarchive, resOutput003.id
            )
            )
            .set(authHeaders)
            .send(sendBody005)
    ...
    it('creative is-archived should be true', function() {
        expect(resOutput005.isArchived).to.eql(false);
    });
