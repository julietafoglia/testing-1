...
describe('{{MERLIN}} /creative/{id details} @ADMIN >>> ' +
    '(+) url - basic verification - ref-id - base 64 file >>>', function() {
    ...
    before('get creative details - ' +
        'basic verification - ref-id - base 64 file', function(done) {

        request(targetServer)
            .get(util.format(
                targetEndpoint.creativeDetails, resOutput003.refId
            )
            )
            .set(authHeaders)
            .then( function(res) {
    ...
    it('response object key values should match test object', function() {
        ...
        // created and modified
        expect(resOutput004.created)
            .to.equal(resOutput003.created);
        expect(resOutput004.createdBy
        ).to.equal(resOutput003.createdBy);
        expect(resOutput004.modified)
            .to.equal(resOutput003.modified);
        expect(resOutput004.modifiedBy)
            .to.equal(resOutput003.modifiedBy);
    });
