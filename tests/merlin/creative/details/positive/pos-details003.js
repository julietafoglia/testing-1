...
describe('{{MERLIN}} <SMOKE> /creative/{id details} @ADMIN >>> ' +
    '(+) url - basic verification - hash id - base 64 file >>>', function() {
    ...
    before('get creative details - ' +
        'basic verification - hash id - base 64 file', function(done) {

        request(targetServer)
            .get(util.format(
                targetEndpoint.creativeDetails, resOutput003.id
            )
            )
            .set(authHeaders)
            .then( function(res) {
