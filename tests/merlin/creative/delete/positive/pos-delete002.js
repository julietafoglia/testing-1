...
describe('{{MERLIN}} /creative/{id delete} @ADMIN >>> ' +
    '(+) url - save verification - url file >>>', function() {
    ...
    before('delete creative', function(done) {
        request(targetServer)
            .del(util.format(
                targetEndpoint.creativeDelete, resOutput003.id
            )
            )
            .set(authHeaders)
            .then( function(res) {
    ...
    before('save creative - update name', function(done) {
        sendBody005 = {};
        sendBody005.version = resOutput003.version;
        sendBody005.name = resOutput003.name + ' A';
    ...
    it('response should have status of 410', function() {
        expect(res005.status).to.equal(410);
    });
