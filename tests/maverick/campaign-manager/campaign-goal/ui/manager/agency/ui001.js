...
describe('{{MAVERICK}} /campaign-manager/campaign-goal {ui} @MANAGER >>> ' +
    '(+) performance - pay per click - ssp - edit >>>', function() {
    ...
    xit('goal page should be displayed', function(done) {
        ...
        // campaignFormPage.clickLinkChangeGoal();
        // expect(campaignCardsPage.verifyBrandingNotPresent()).to.be.true;
        ...
        // expect(campaignCardsPage.verifyMaxReachNotPresent()).to.be.true;
        // expect(campaignCardsPage.verifyPayPerConvNotPresent()).to.be.true;
        // expect(campaignCardsPage.verifyMaxClickNotPresent()).to.be.true;
        driver.sleep(driverTimeOut)
            .then(() => done());
    });
