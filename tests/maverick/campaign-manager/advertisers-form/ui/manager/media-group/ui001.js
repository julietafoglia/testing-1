...
describe('<UNSTABLE> {{MAVERICK}} /advertisers {ui} @MANAGER >>> ' +
    ...
    it('should check ui elements', function(done) {
        ...
        advertiserPage.getInputLabel().then(function(element) {
            expect(element).to.exist;
        });
        advertiserPage.closeInputLabel().click();
        advertiserPage.getLinkWhitelist().then(function(element) {
            expect(element).to.exist;
        });
        ...
        advertiserPage.getTextAccountManager().getAttribute('class').
            then(function(element) {
                expect(element).to
                    .not.contain('readonly-text');
            });
