...
const targetCampaign = targetIo.children.dspCampaigns[4];
...
testData001.name = 'SC8 ' + testData001.name + timeStamp;
...
describe('<SMOKE> {{MAVERICK}} /line-item {create} @MANAGER >>> ' +
    '(+) body - minimum required - EX - perf max clicks SC8 daily >>>',
function() {
    ...
    it('SC8 line item should be created - minimum required', function(done) {
        lineItemPage.setName(testData001.name);
        lineItemPage.setDailyBudget(testData001.budget);
