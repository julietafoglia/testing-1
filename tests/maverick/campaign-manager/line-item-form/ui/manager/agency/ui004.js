...
const timeMonth = moment().add(30, 'days').format('M/D/YY');
...
describe('{{MAVERICK}} /line-item {UI} @MANAGER >>> ' +
    ...
    it('should fill all data', function(done) {
        ...
        lineItemPage.setInputSearchLocations('United States');
    ...
    it('should show correct data in review page basic details', function(done) {
        ...
        lineItemReviewPage.getTextPacing().getAttribute('innerHTML').
            then(function(innerHTML) {
                expect(innerHTML).to
                    .equal('Even Pacing');
            });
    ...
    it('should show correct data in review page targeting', function(done) {
        ...
        lineItemReviewPage.getTextWhitelistTarget().getAttribute('innerHTML').
            then(function(innerHTML) {
                expect(innerHTML).to
                    .equal('Targeted');
            });
        lineItemReviewPage.getTextWhitelistCount().getAttribute('innerHTML').
            then(function(innerHTML) {
                expect(innerHTML).to
                    .equal('1');
            });
        lineItemReviewPage.getTextWhitelistSelection().
            getAttribute('innerHTML').then(function(innerHTML) {
                expect(innerHTML).to
                    .contain(targetPublisher);
            });
        driver.sleep(driverTimeOut).then(() => done());
    });
    ...
    it('should show correct data in review page targeting' +
        ...
        lineItemReviewPage.getTextBlacklistTarget().getAttribute('innerHTML').
            then(function(innerHTML) {
                expect(innerHTML).to
                    .equal('Excluded');
            });
        lineItemReviewPage.getTextBlacklistCount().getAttribute('innerHTML').
            then(function(innerHTML) {
                expect(innerHTML).to
                    .equal('1');
            });
        lineItemReviewPage.getTextBlacklistSelection()
            .getAttribute('innerHTML').
            then(function(innerHTML) {
                expect(innerHTML).to
                    .contain(targetPublisher);
