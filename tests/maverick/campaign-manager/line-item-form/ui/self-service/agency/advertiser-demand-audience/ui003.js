...
describe('{{MAVERICK}} /line-item {UI} @SS-AGENCY-ADVERTISER >>> ' +
    ...
    it('should display selected ads text', function(done) {
        lineItemPage.getExistingAdCheckbox().click();
        lineItemPage.getTextSelectedAd('1 item selected')
        ...
        lineItemPage.getTextSelectedAd('3 items selected')
            .getAttribute('innerHTML').
            then(function(innerHTML) {
                expect(innerHTML).to
                    .equal('3 items selected');
            });
        driver.sleep(driverTimeOut).then(() => done());
    });
