...
describe('<UNSTABLE> {{MAVERICK}} /conversion-trackers {create} @MANAGER >>> ' +
    '(+) create simple tracker >>>', function() {
    ...
    it('should fill modal with valid data', function(done) {
        ...
        trackerPage.selectImagePixel();
        trackerPage.clickTrackerCreate();
        driver.sleep(driverTimeOut).then(() => done());
    });

    it('should assert conversion tracker displayed in table', function(done) {
        trackerPage.getAlert();
        trackerPage.closeAlert();
        trackerPage.clickTableHeaderCreated();
        trackerPage.clickTableHeaderCreated();
        trackerPage.getTableTracker(tracker.name);
        driver.sleep(driverTimeOut).then(() => done());
    });
