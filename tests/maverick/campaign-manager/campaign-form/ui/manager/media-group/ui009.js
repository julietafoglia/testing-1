...
describe('<UNSTABLE> {{MAVERICK}} /campaign-manager/campaign-form {ui} ' +
    '@MANAGER >>> (+) verify ui elements - direct sold - branding ' +
    ...
    it('should contain all elements for ' +
        'direct sold - branding - impressions', function(done) {
        ...
        campaignFormPage.getInputGuaranteed().then(function(element) {
            expect(element).to.exist;
        });
        campaignFormPage.clickGuaranteed();
        campaignFormPage.getInputShareOfVoice().then(function(element) {
            expect(element).to.exist;
        });
        campaignFormPage.getButtonBudgetType().then(function(element) {
            expect(element).to.exist;
        });
        campaignFormPage.clickButtonBudgetType();
        campaignFormPage.getSpan('Currency').then(function(element) {
            expect(element).to.exist;
        });
        campaignFormPage.getSpan('Impressions').then(function(element) {
            expect(element).to.exist;
        });
