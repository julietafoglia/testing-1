...
testData001.name = testData001.name + timeStamp +
    ' (ssp - hc - perf max reach)';

describe('<SMOKE> {{MAVERICK}} /campaign {create} @MANAGER >>> SSP ' +
    '(+) house - performance - max reach - minimum required >>>', function() {
    ...
    it('maverick - navigate to campaign form - ' +
        'performance - max reach', function(done) {
        ...
        campaignCardsPage.clickMaxReach();
        driver.sleep(driverTimeOut)
            .then(() => done());
    });

    it('campaign should be created - ssp - house - perf max reach',
        function(done) {
            ...
            campaignFormPage.setTargetBidAmount(testData001.bidAmount);
