...
testData001.name = testData001.name + timeStamp +
    ' (dsp - performance - pay per conversion)';

describe('<SMOKE> {{MAVERICK}} /campaign {create} @MANAGER >>> ' +
    'DSP (+) performance - pay per conversion - minimum required >>>',
function() {
    ...
    it('campaign should be created - dsp - exchange - performance -' +
        '  pay per conversions', function(done) {
        ...
        campaignCardsPage.clickPayPerConv();
        ...
        expect(campaignFormPage.getInputCrx()).to.exist;
        campaignFormPage.getInputName().click();
