...
describe('{{SALESFORCE}} client direct - direct advertiser - {create} >>> ' +
    '(+) pay per conversion - with targeting >>>', function() {
    ...
    describe('salesforce - create a direct advertiser opp >>>', function() {
        ...
        productFixture = Object.assign(
            {}, require(rootPath + '/fixtures/salesforce/product-create005')
        );
    ...
    describe('merlin - verify pixel was created', function() {
        ...
        before('advanced search - pixel', function(done) {
            let payload = {
                'conditions': [
                    {'field': 'name', 'value': productFixture.pixelName}
                ]
            };
        ...
        it('pixel should be linked to campaign', function() {
            expect(resOutput007[0].name)
                .to.equal(productFixture.pixelName);
            expect(resOutput007[0].advertiser)
                .to.equal(resOutput002[0].id);
            expect(resOutput007[0].advertiserName)
                .to.equal(resOutput002[0].name);
            expect(resOutput007[0].id)
                .to.equal(resOutput006.conversionPixel);
        });
    });
